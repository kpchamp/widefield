...
class DynamicModeDecomposition:
    ...
    def fit(self, X, U=None, dynamics_rank=None):
        if X.ndim == 3:
            ...
            n_trials, n_timesteps, self.n_features = X.shape
            n_samples = n_trials*n_timesteps
            X_left = X[:,1:,:].reshape((n_samples-n_trials, self.n_features)).T
            X_right = X[:,:-1,:].reshape((n_samples-n_trials, self.n_features)).T
        elif X.ndim == 2:
